...
#+BEGIN_SRC emacs-lisp
  ...
  (define-key org-mode-map (kbd "\C-c ]") 'zxmth/org-archive-complete-tasks)
#+END_SRC
...
followed by my tasks scheduled for the current day and finally, all of my TODOs
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun zxmth/pop-to-org-agenda (&optional split &optional command)
    "Visit the org agenda in the current window or a SPLIT
     Opening in current window causes other windows to be deleted
     COMMAND key tells org-agenda to open specific view."
    (interactive "P")
    (unless command (setq command "d"))
    (org-agenda nil command);; open "Daily Agenda" custom command
    (when (not split)
      (delete-other-windows)))
  (global-set-key (kbd "C-x !") 'zxmth/pop-to-org-agenda)
#+END_SRC
* Git
